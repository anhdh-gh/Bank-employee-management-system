...
if (urlsPageNotAuth.every((url) => url !== pathname)) {
  if (!Cookies.get("token") && !sessionStorage.getItem("token")) backLogin();
  else {
    ApiClient.get("/person/info")
      .then((resp) => {
        ...
        if (role.some((i) => i === "Customer")) {
          if (!pathname.includes("/customer")) backLogin();
        } else {
          if (!pathname.includes("/employee")) {
            backLogin();
          } else {
            if (role.some((i) => i === "Employee")) {
              if (privateManager.some((i) => pathname.includes(i))) backLogin();
              else {
        ...
        $("#view-profile").attr(
          "href",
          "profile.html?id=" + info.id + "&role=" + role[0]
        );
      })
      .catch((err) => {
        console.log(err);
